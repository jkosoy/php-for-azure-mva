<?php
	/*
	 * this example builds off our previous to add in sessions. sessions are basically cookies stored on the server.
	 * here we see a new function, session_start(), as well as the $_SESSION super global.
	 */

	session_start(); // starts a session. always makes sure this is at the top!
	...
	elseif($page == 'rsvp') {
		// if has_rsvped exists in our super global the user must've already submitted. show a different template.
		if(isset($_SESSION['has_rsvped'])) {
			$page = 'already-rsvped';
		}
		else {
			$rsvpError = false;
