<?php
	/*
	 * this example combines our cookie script from earlier with the MySQL write example we just learned.
	 */

	// connect to our database and select it
	...
	if(isset($_GET['killcookie'])) {
		unset($_COOKIE['name']);
		unset($_COOKIE['has_rsvped']);

		setcookie("name", null, -1, "/");		
		setcookie("has_rsvped", null, -1, "/");		
	}
		...
		if(isset($_COOKIE['has_rsvped'])) {
			...
			if(isset($_POST['name'])) {
				...
				if(!$rsvpError) {
					...
					// sanitize the name and their guest with real_escape_string.
					$name = $mysql->real_escape_string($_POST['name']); 
					$guest = $mysql->real_escape_string($_POST['date_name']);

					// and make sure attending is either false or true.
					$attending = false;
					...
					$query = "INSERT INTO `rsvps` (name,attending,guest) VALUES ('$name', '$attending', '$guest');";
					$mysql->query($query);
					...
					setcookie("has_rsvped", true, $expiration, "/");
				}
			}			
		}
